...
class Storage:
    ...
    def save(self, image: bytes, image_name: str) -> None:
        image_path = Path(self._config["dir_path"]) / f"{image_name}.jpg"
        _ = cv2.imwrite(
            str(image_path),
            image
        )
...
class ProcessGranules:
    ...
    def __init__(
        self,
        storage: Storage,
        granules_mask_predictor: GranulesPredictor
    ):
        self.storage = storage
        self.granules_mask_predictor = granules_mask_predictor
        print("finish process plate init")

    def process(self, image: np.ndarray, content_name: str) -> dict:
        """
        Process image
